...
def recommend(song, song_list, similarity, n_recommendations=5):
    try:
        ...
        for i in distances:
            song_index = i[0]
            if song_index not in recommended_indices: 
                recommended_music_names.append(song_list.iloc[song_index]['track_name']) 
                recommended_music_track_ids.append(song_list.iloc[song_index]['track_id']) 
                recommended_indices.add(song_index)
                if len(recommended_music_names) >= n_recommendations:
                    break
